...
.plot_full_fov <- function(df_in, fov_in, color, x = "x_cell", y = "y_cell",
                           ...
                           trace_size = 2, trace_clr = "black", show_n = TRUE) {
...
.plot_full_fov_signal <- function(df_in, fov_in, color, x = "x_cell", y = "y_cell",
                                  clrs = c("white", "#D7301F"),
                                  size = 1, pseudo = 1, trans = "log10",
                                  ttl = NULL, signal_size = 2, ...) {
  
  highlight_signal <- signal_size != size
  ...
  if (highlight_signal) {
    res <- res +
      geom_point(
        aes(color = !!sym(color) + pseudo),
        data = trace_dat,
        size = signal_size
      ) +
      scale_fill_gradientn(colours = clrs[1], trans = trans) +
      scale_color_gradientn(colours = clrs, trans = trans) +
      guides(
        color = guide_colorbar(ticks = FALSE, title = ttl),
        fill = "none"
      ) +
      expand_limits(color = pseudo)
