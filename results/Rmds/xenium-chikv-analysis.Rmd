...
params:
  ...
  colors:       "~/Projects/morrison-chronic-chikv/ref/celltype_colors.qs"
...
* Adjusted colors for cell types
* Increased point size for [high resolution plots](#hires) showing CHIKV counts
  and CHIKV+ cells
...
```{r "LOAD PARAMS INTERACTIVELY", eval = FALSE}
# This is needed to correctly load params into environment when using Positron
collapse_list <- function(x) {
  # If it's a list with only one element that is also a list, collapse it
  while (is.list(x) && length(x) == 1 && is.list(x[[1]])) {
    x <- x[[1]]
  }
  
  # Recursively apply to all elements if still a list
  if (is.list(x)) {
    x <- lapply(x, collapse_list)
  }
...
params <- rmarkdown::yaml_front_matter(rmd)$params
params <- collapse_list(params)
```
...
# Cell type colors
# * load from file
# * use the most distinct colors for the high confidence cell types that will
#   be included in the main figures
typ_clrs <- qread(here(params$colors))

main_clrs <- typ_clrs[main_types]
...
# * include the "other" group
type_order <- xen_dat$cell_type_2 %>%
  table() %>%
  sort(decreasing = TRUE) %>%
  names()

type_order <- unique(c(type_order, cell_n$cell_type))
type_order <- unique(c(type_order, "Macrophages"), fromLast = TRUE)
...
ggsave(
  file.path(fig_dir, "cell_type_sections.pdf"),
  plot = fig, device = cairo_pdf, width = 18, height = 15
)
```
...
fovs %>%
  walk(~ {
    ...
    print(fig)
    ggsave(
      file.path(fig_dir, str_c("top_cell_types_sections_", params$xen_sample_names[[.x]], ".pdf")),
      plot = fig, device = cairo_pdf, width = 16, height = 8
    )
    cat("\n\n<br>\n\n<br>\n\n")
  })
```
...
fovs %>%
  walk(~ {
    ...
    print(fig)
    ggsave(
      file.path(fig_dir, str_c("hi_res_cell_types_sections_", params$xen_sample_names[[.x]], ".pdf")),
      plot = fig, device = cairo_pdf, width = 45, height = 10
    )
    cat("\n\n<br>\n\n<br>\n\n")
  })
```
...
fovs %>%
  walk(~ {
    ...
    print(fig)
    ggsave(
      file.path(fig_dir, str_c("xenium_cell_type_sections_", params$xen_sample_names[[.x]], ".pdf")),
      plot = fig, device = cairo_pdf, width = 14, height = 12
    )
    cat("\n\n<br>\n\n<br>\n\n")
  })
```
...
ggsave(
  file.path(fig_dir, "pred_confidence.pdf"),
  plot = fig, device = cairo_pdf, width = 9, height = 4
)
```
...
plts %>%
  iwalk(~ {
    ...
    print(.x)
    ggsave(
      file.path(fig_dir, str_c("cell_type_markers_", .y, ".pdf")),
      plot = .x, device = cairo_pdf, width = 15, height = 15
    )
    cat("\n\n<br>\n\n<br>\n\n")
  })
```
...
marks %>%
  iwalk(~ {
    ...
    print(plt)
    ggsave(
      file.path(fig_dir, str_c("cell_type_marker_degs_", .y, ".pdf")),
      plot = plt, device = cairo_pdf, width = 15, height = 8
    )
    cat("\n\n---\n\n<br>\n\n<br>\n\n")
  })
```
...
ggsave(
  file.path(fig_dir, "chikv_pos_cell_types.pdf"),
  plot = fig, device = cairo_pdf, width = 17, height = 11
)
```
...
fovs %>%
  walk(~ {
    ...
    print(fig)
    ggsave(
      file.path(fig_dir, str_c("top_cell_types_chikv_pos_sections_", params$xen_sample_names[[.x]], ".pdf")),
      plot = fig, device = cairo_pdf, width = 16, height = 8
    )
    cat("\n\n<br>\n\n<br>\n\n")
  })
...
fovs %>%
  walk(~ {
    ...
    ggsave(
      file.path(fig_dir, str_c("hi_res_chikv_sections_", params$xen_sample_names[[.x]], ".pdf")),
      plot = fig, device = cairo_pdf, width = 45, height = 10
    )
    cat("\n\n")
  })
...
fovs %>%
  walk(~ {
    ...
    print(fig)
    ggsave(
      file.path(fig_dir, str_c("chikv_pos_heatmap_sections_", params$xen_sample_names[[.x]], ".pdf")),
      plot = fig, device = cairo_pdf, width = 14, height = 10
    )
    cat("\n\n<br>\n\n<br>\n\n")
  })
```
...
ggsave(
  file.path(fig_dir, "chikv_counts_cell_types.pdf"),
  plot = fig, device = cairo_pdf, width = 15, height = 6
)
```
...
ggsave(
  file.path(fig_dir, "chikv_signal_cell_types.pdf"),
  plot = fig, device = cairo_pdf, width = 15, height = 4
)
```
...
ggsave(
  file.path(fig_dir, "chikv_pos_bargraphs_paper.pdf"),
  plot = fig, device = cairo_pdf, width = 8, height = 4
)
```
...
ggsave(
  file.path(fig_dir, "chikv_pos_bargraphs.pdf"),
  plot = fig, device = cairo_pdf, width = 10, height = 4
)
```
...
ggsave(
  file.path(fig_dir, "chikv_signal_types.pdf"),
  plot = fig, device = cairo_pdf, width = 15, height = 4
)
```
...
ggsave(
  file.path(fig_dir, "chikv_signal_groups_paper.pdf"),
  plot = fig, device = cairo_pdf, width = 6, height = 4
)
```
...
ggsave(
  file.path(fig_dir, "chikv_signal_groups.pdf"),
  plot = fig, device = cairo_pdf, width = 10, height = 5
)
```
...
.create_prox_bars <- function(df_in, cell_type = "Macrophages", seed = 42, force = 1, nudge_y = 1, clrs = c(
    `CHIKV+` = "#D7301F",
    `CHIKV-` = "#53B3E1",
    none     = typ_clrs[["low"]]
  )) {
...
ggsave(
  file.path(fig_dir, "proximity_Macrophages.pdf"),
  plot = fig, device = cairo_pdf, width = 15, height = 3.75
)
```
...
typs %>%
  iwalk(~ {
    ...
    plt <- prox_df %>%
      .create_prox_bars(.y, seed = 42, nudge_y = 1.5, force = 5)

    print(plt)

    ggsave(
      file.path(fig_dir, str_c("proximity_", .y, ".pdf")),
      plot = plt, device = cairo_pdf, width = 15, height = 3.75
    )
    cat("\n\n---\n\n<br>\n\n<br>\n\n")
  })
```
