...
```{r "giotto setup"}
run_giotto <- !file.exists(here("results/prox.tsv.gz"))
...
```{r "giotto object", include = FALSE, eval = run_giotto}
...
prox_res <- gio_dirs %>%
  imap(~ {
    ...
    obj %>%
      qsave(here(str_c("results/gio_", nm, ".qs")))
    ...
    prox %>%
      qsave(here(str_c("results/prox_", nm, ".qs")))
