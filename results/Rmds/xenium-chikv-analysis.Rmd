...
All CHIKV+ cells are shown, all other cells were downsampled for visualization.
...
ht <- ht_dat %>%
  ...
  scale_y_continuous(labels = label_percent(), expand = expansion(c(0.05, 0.25))) +
...
Only cell types with at least 250 cells are shown, cells with no assigned
cell type are excluded.

```{r "xenium cell type sections", fig.width = 14, fig.height = 14, results = "asis"}
fovs %>%
  walk(~ {
    cat("\n\n####", params$xen_sample_names[[.x]], "\n\n")
    
    # Format data for plots
    # * only show cell types with >100 cells
    # * do not show unassigned cells
    plt_dat <- xen_dat %>%
      filter(fov == .x)
    
    xen_typs <- plt_dat$cell_type %>%
      table() %>%
      sort(decreasing = TRUE)
    
    xen_typs <- xen_typs[xen_typs >= 250]
...
```{r "CHIKV+ heatmap sections", fig.width = 14, fig.height = 14, results = "asis"}
fovs %>%
  walk(~ {
    cat("\n\n####", params$xen_sample_names[[.x]], "\n\n")
    ...
    # Create plots
    typ_plts <- names(sort(chikv_types, decreasing = TRUE)) %>%
      map(~ {
        typ <- .x
...
```{r "T cell proximity", include = FALSE, eval = create_giotto}
prox_res <- gio_dirs[1] %>%
  imap(~ {
    # Create Giotto object
    # * store custom targets as separate assay
