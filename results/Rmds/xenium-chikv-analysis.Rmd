...
Expression of marker genes is shown below.
...
High-resolution images show CHIKV counts for each tissue section.
...
dat <- xen_dat %>%
  ...
  mutate(
    ...
    across(
      c(n, n_pos),
      ~ if_else(.x >= 1000, str_c(round(n / 1000, 1), "k"), as.character(.x))
    ),
    n_lab     = str_c(n_pos, "/", n),
